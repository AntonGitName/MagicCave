...
public class CandleGraphView {
    ...
    public CandleGraphView(ResourceLoader rl) {
        ...
        for (int i = 0; i < candles.length; ++i) {
            candles[i] = new CandleView(candleModels[i], rl.getCandleBitmap(new Point(30, 40)), rl.getFireBitmaps(new Point(20, 40)));
        }
    }
    ...
    public void draw(Canvas canvas, Paint linePaint, int w, int h) {
        for (CandleView candle : candles) {
            ...
            /*
             * CandleModel m = candle.getModel(); float x1 = w * m.getX(); float
             * y1 = h * m.getY(); float x2; float y2; for (CandleModel neighbour
             * : m.getNeighbours()) { x2 = w * neighbour.getX(); y2 = h *
             * neighbour.getY(); canvas.drawLine(x1, y1, x2, y2, linePaint); }
             */
        }
    }
}
